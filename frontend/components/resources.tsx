              ...
              <select
                ...
                onChange={(event) =>
                  setChoices({
                    ...choices,
                    grouping:
                      event.target.value as DataChoices["grouping"],
                  })
                }
              >
              ...
              <select
                ...
                onChange={(event) =>
                  setChoices({
                    ...choices,
                    aggregation:
                      event.target.value as DataChoices["aggregation"],
                  })
                }
              >
              ...
              <select
                ...
                onChange={(event) =>
                  setChoices({
                    ...choices,
                    chart:
                      event.target.value as DataChoices["chart"],
                  })
                }
              >
...
export function VideoPanel({
  ...
  const [bookmarkTimestamp, setBookmarkTimestamp] =
    useState(0);

  const [bookmarkNote, setBookmarkNote] =
    useState("");

  const [showBookmarkForm, setShowBookmarkForm] =
    useState(false);

  const [savingBookmark, setSavingBookmark] =
    useState(false);
  ...
  /*
   * Load saved bookmarks for this session.
   */
  useEffect(() => {
    let active = true;

    api<VideoBookmark[]>(
      `/sessions/${sessionId}/video-bookmarks`,
    )
      .then((result) => {
        if (active) {
          setBookmarks(result);
   ...
   *
   * IMPORTANT:
   * We do NOT put the newly-created player into the ref
   * until YouTube fires onReady.
   *
   * This prevents:
   *
   * youtubePlayerRef.current.seekTo is not a function
   */
  useEffect(() => {
    if (
      !selectedVideo ||
     ...
     * here.
     *
     * The player becomes available through onReady.
     */
    void player;
  ...
  /*
   * If the user changes the start time while the player
   * is already ready, seek safely.
   */
  useEffect(() => {
  ...
  async function searchYouTube(
    ...
    if (!query) {
      setError(
        "Enter a topic to search for YouTube videos.",
      );
      return;
    }
  ...
  async function saveSelectedVideo() {
    ...
    if (
      ...
      setError(
        "Timestamp must be between 0 and 86400 seconds.",
      );
      return;
    }
    ...
    try {
      const video = await postJson<Video>(
        "/videos/segments",
        {
  ...
  async function addManualVideo(
    ...
    const start = Number(
      form.get("start_seconds"),
    );

    const end = Number(
      form.get("end_seconds"),
    );
  ...
  /*
   * Get a timestamp from the active YouTube player.
   */
  function bookmarkCurrentMoment() {
    const player = youtubePlayerRef.current;
    ...
    if (
      ...
      setError(
        "The YouTube player is not ready yet. Please wait a moment and try again.",
      );
      return;
    }
  ...
  /*
   * Save bookmark to the backend.
   */
  async function saveBookmark() {
    if (locked || !selectedVideo) {
      return;
    }
    ...
    if (!note) {
      setError(
        "Add a short note for this bookmarked moment.",
      );
      return;
    }
  ...
  return (
    <section className="content-card">
      ...
      <p className="muted">
        Search YouTube for videos related to your
        current learning topic, choose a useful
        video, and save the exact section you want
        to revisit.
      </p>
      ...
      {selectedVideo && (
        <div className="selected-video">
          ...
          {/* Actual YouTube IFrame API player */}
          ...
          <div className="section-heading">
            <div>
              ...
              <p className="muted">
                Play the video, pause at the
                useful moment, then bookmark
                that exact position with a note.
              </p>
            </div>
            ...
            <div className="reference-card">
              <label>
                ...
                <textarea
                  ...
                  placeholder="What should you remember from this moment?"
                />
              </label>
              ...
              <div className="inline">
                ...
                >
                  {savingBookmark ? (
                    <BusyLabel text="Saving bookmark…" />
                  ) : (
                    <>
                      <Bookmark
                        size={16}
